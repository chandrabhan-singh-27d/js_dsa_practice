...
Beacuse, in the first place we checked for the length of both the strings. Then on, we assigned value to the letters of the first string and we mapped the string as an object with the number of instances of the letters. Now, we need to check if the exact same letters are present in the second string with the same number of repetitions or not. So, for that, we will first check if the charObj contains letters of the second string or not, if not then return false. If yes, then we need to check the number of times those letters are present in the second string and for that we will need to decrease the value, once it reaches zero and if there is still a letter present in the second string it means that the map object we created does not sync with the chars of string2 and hence will return false, else will return true. 
*/
